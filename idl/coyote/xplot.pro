...
;
; Copyright © 1997-2005 Fanning Software Consulting.
;
...
CASE whichFileType OF
   ...
   ELSE: BEGIN
      filename = Dialog_Pickfile(/Write, File='xplot.eps')
      IF filename NE '' THEN BEGIN
         CASE whichFileType OF
            'EPS-72' : resolution = [2.54/72, 2.54/72]
            'EPS-150': resolution = [2.54/150, 2.54/150]
            'EPS-300': resolution = [2.54/300, 2.54/300]
         ENDCASE
         info.thisWindow->GetProperty, Dimensions=theDimensions, Units=theUnits
         clipboard = Obj_New('IDLgrClipboard', Dimensions=theDimensions, $
